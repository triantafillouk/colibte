...
void bnf_factor_quote()
{
	...
	bnf_var->var_alloced=0;
	// MESG("bnf_factor_quote: put quote '%s' var@=%d type %d",bnf_var->sval,VARIND,bnf_var->var_type);
}
...
inline static void bnf_factor_sep()
{
 if(bnf_var-bnf_vars>1) 
 { 
	MESG("bnf_factor_sep: set prev_var [%s]",tok_info(tok));
	prev_var("sep");
...
void bnf_factor_line_array1()
{
	...
	MESG("bnf_factor_line_array1:%d var@=%d Array definition ------------",adat->anum,VARIND);
	allocate_array(adat);
	NTOKEN2;
	while(cdim>0){

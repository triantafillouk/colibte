...
#endif 
	if (    tp_offset(wp->tp_hline) > tp_offset(wp->tp_hmknown) 
				&& tp_offset(wp->tp_hline) - tp_offset(wp->tp_hmknown) < TRACE_BACK) 
	{
...
//		if(single_quoted) { hstate=0;break;};
	...
	case '#':
		...
		if(hquotem&H_QUOTEC) { hstate=0;break;};
  ...
  switch(c) {
	...
	case CHR_CR:
		...
		if(hprev_line>0 && slang) { 
			...
			MESG("CHR_LINE: hquotem=%d slang=%d",hquotem,slang);
		} else {
			//hquotem &= ~(H_QUOTE1|H_QUOTE4|H_QUOTE5|H_QUOTE6|H_QUOTE10|H_QUOTE11|H_LINESEP);
			hquotem=0;
			hprev_line=-1;
			MESG("CHR_LINE: reset , slang=%d",slang);
		};
#endif
		hstate=HS_LINESTART;
		h_line_set=0;
	...
	case '`': // code block 
		...
		if(code_start==3) {
			if(hprev_line>0) {
				slang=0;
				hprev_line=-1;
				MESG("code_block: stop code!");
				hquotem=0;
			} else {
				slang=LANG_SCRIPT;
				hprev_line=H_QUOTE12;
				MESG("code_block: start code!");
				hquotem=0;
			};
			break;
		};
#endif
#if	0
		if(code_start == 1 && slang==0) {
			slang=1;
			prev_set = hquotem | H_QUOTE11;
			hstate=0;
			break;
		};
		if(code_start == 2 && slang==1) {
			slang=0;
			hquotem &= ~H_QUOTE11;
			break;
		};

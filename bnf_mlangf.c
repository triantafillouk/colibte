...
void bnf_print()
{
	...
	// MESG("bnf_print: << tnum=%d args=%d var@=%d",tok->tnum,tok->t_nargs,VARIND);
	int i;
...
void bnf_test_loop()
{
	long int lmax= (long int) bnf_var->dval;
	double p=0;
	double l=1;
	int j=1;
	int i=1;
...
void bnf_to_num_array()	/* TBC  */
{
	...
	if(bnf_var->var_type==VTYPE_AMIXED){
		array_dat *a_old=bnf_var->adat;

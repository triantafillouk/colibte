...
cairo_t *begin_plot(GePlotArea  *wd,cairo_region_t *region,char *message)
{
 unsigned long lwd;
 lwd = (unsigned long) (wd->plot_window);
	if(wd->in_draw) { 
		return wd->cr;
	}
	if(wd->cr !=NULL) {
	...
	wd->plot_gdk_context = gdk_window_begin_draw_frame (wd->plot_window,region);
	wd->cr = gdk_drawing_context_get_cairo_context (wd->plot_gdk_context);
	return wd->cr;
}

void end_plot(GePlotArea *wd,char *message)
{
 unsigned long lwd;
 lwd = (unsigned long) (wd->plot_window);
	if(wd->in_draw) { 
	...
	gdk_window_end_draw_frame(wd->plot_window,wd->plot_gdk_context);
	wd->cr=NULL;
}
...
void redraw(GtkWidget *widget)
{
 ...
 int coords_before=0;
	...
	region = gdk_window_get_clip_region (wd->plot_window);
	cr = begin_plot(wd,region,"redraw");
...
static gboolean clicked(GtkWidget *widget, GdkEventButton *event)
{
 char st[128];
    if (event->button == 1) {
		...
		snprintf(st,128,"add point[%d] (%f %f)\n",glob.count,event->x, event->y);
		plot_status(1,st);
		glob.count++;
...
int plot_init(int n)
{
  ...
  if(wplotp==NULL) {
	...
	gtk_window_set_default_size(GTK_WINDOW(wplotp), plot_width+32,plot_height+16);
	gtk_window_set_resizable(GTK_WINDOW(wplotp),TRUE);
	...
	gtk_widget_realize(toolbar_plot);
	hbox1 = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 0);
	gtk_box_pack_start(GTK_BOX(vbox1),hbox1,FALSE,FALSE,0);
	set_box_color(hbox1,"black","grey50");
	...
	set_box_color(gtk_statusbar_get_message_area ((GtkStatusbar *)(wplot_sline)),"blue","gray");
#endif
	wplot_pline = gtk_statusbar_new();
	gtk_widget_show(wplot_pline);
	gtk_box_pack_start(GTK_BOX(hbox1),wplot_pline,TRUE,TRUE,0);
	gtk_widget_show(wplot_pline);
	set_box_color(gtk_statusbar_get_message_area ((GtkStatusbar *)(wplot_pline)),"white","gray");
	...
	g_signal_connect_object(G_OBJECT(wplotp), "destroy", G_CALLBACK(plot_close), G_OBJECT(wplotp), G_CONNECT_AFTER);
	
	g_signal_connect(darea, "button-press-event",  G_CALLBACK(clicked), NULL);
	g_signal_connect(darea, "motion_notify_event", G_CALLBACK(on_plot_motion), G_OBJECT(darea));
	gtk_window_set_title(GTK_WINDOW(wplotp), "Lines");
	gtk_widget_show(wplotp);
 ...
 v = add_plot_element(PTITLE,1+strlen(title)/sizeof(double) );
 strlcpy((char *)v, title,strlen(title)+1);
 };
#endif
 return true;
}

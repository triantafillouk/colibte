...
int err_skip_type_args(tok_struct *tok0)
{
 ...
 if(tok->ttype==TOK_LBRAKET) {
 	// MESG("	skip index");
	stack_push("type lbraket",tok);
	NTOKEN_ERR(661);
	stat=err_cexpression();
	stack_push("type_args sep",tok);
	NTOKEN_ERR(662);
 };

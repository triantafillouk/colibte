...
void set_error(tok_struct *tok,int err,char *description)
{
 ...
 tok->tgroup=TOK_END;
 // tok->ttype=TOK_EOF;
 set_ttype("set error",tok,TOK_EOF);
...
#endif
{
...
#if	TBNF
#if	!TFUNC 
			if(tok->ttype==TOK_RPAR) stack_push("eval_function )",tok,-TOK_RPAR,&new_one);
#endif
#endif
			break;
		}
	...
	// MESG("		after argument loop ia=%d ia0=%d [%s]",ia,ia0,tok_info(tok));
	// Check for correct number of args, this sould not happen!
	if(ia0>=0 && ia0!=ia) {
		MESG("Error in number of args: %d != %d",ia0,ia);
		syntax_error(4031,"function arguments error");
		return(err_num);
	};
	// MESG("	err_eval_fun_1: 1");
	if(tok->ttype==TOK_RPAR) {
		// MESG("	RPAR: [%s]",tok_info(tok));
#if	TBNF
#if	!TFUNC
		// MESG("err_eval_fun1: push RPAR!!");
		if(lpar) stack_push("eval_func )",tok,-TOK_RPAR,&new_one);
#endif
#endif
		NTOKEN_ERR(404);
		// MESG("	eval_func1: after rpar: [%s]",tok_info(tok));
		// if(tok->ttype==TOK_SEP) stack_push("function end",tok,0,&new_one);
	};
	...
	stack_push("eval_func",tok0,TOK_FUNC,&new_one);
#else
...
int err_skip_type_args(tok_struct *tok0)
{
 ...
 if(tok->ttype==TOK_LBRAKET) {
 	// MESG("	skip index");
	stack_push("type lbraket",tok,-TOK_LBRAKET,&new_one);
	NTOKEN_ERR(661);
	stat=err_cexpression();
	stack_push("type_args sep",tok,0,&new_one);
	NTOKEN_ERR(662);
 };
 if(tok->ttype==TOK_LPAR) {
	...
	while(tok->ttype!=TOK_RPAR){
		...
		stack_push("skip args sep",tok,0,&new_one);
		NTOKEN_ERR(668);
		stat=err_cexpression();
	...
	stack_push("skip_args )",tok,0,&new_one);
	NTOKEN_ERR(670);
...
int  err_push_args_1(char *proc_name,int *nargs)
{
 // int i=0;
 int num_args=0;
#if	!TFUNC3
 ...
 SHOW_STAGE(410);
 // MESG("err_push_args: proc=%s b=%s [%s]",proc_name,check_buffer->b_fname,tok_info(tok));
 if(tok->ttype!=TOK_RPAR) {
	if(tok->ttype==TOK_SEP) {
		*nargs=0;
...
int err_assign_args1(int nargs)
{
 ...
 SHOW_STAGE(420);
 stack_push("assign args",tok,0,&new_one); // ????
 ...
 if(nargs!=0) {
	...
	for(i=0;i<nargs;i++) {
		xpos=422;
		if(tok->ttype==TOK_RPAR) {
			// stack_push("end of assign",tok,TOK_RPAR,&new_one);
		...
		if(tok->ttype==TOK_RPAR)  {
			// stack_push("end of assign1",tok,TOK_RPAR,&new_one);
			break;
		}
		...
		stack_push("assign_args sep",tok,0,&new_one);
	...
	while(1) {
		...
		xpos=431;
		stack_push("skip sem par",tok,0,&new_one);
...
int err_increaseby()
{
 TDSERR("increaseby");
 SHOW_STAGE(443);
 // MESG("# increaseby: %s",tok_info(tok));
	if(!simple){
		xpos=443;
		syntax_error(xpos,"bogus assignement!!!!");
		RT_MESG1(443);
...
int err_exec_function(char *name,int nargs,FILEBUF **bf)
{
 ...
 int save_stage_level=stage_level;
    ...
    if ((bp=get_filebuf(bufn,NULL,0)) == NULL) 
	{
		...
		syntax_error(xpos,"function not found");
		RT_MESG;
    };
	*bf=bp;
	// tok_struct *tok1=tok;
...
tok_struct *tok0_bnf=NULL;
...
#endif
 	tok0_bnf=stack_push("factor",tok,tok->ttype,&new_one);	// ????
 };
#endif
 int save_macro_exec;
 tok_struct *tok0; 
 ...
 if(tok0->ttype==TOK_VAR && (tok->ttype != TOK_ASSIGN && tok->ttype!=TOK_INCREASEBY && tok->ttype!=TOK_DECREASEBY\
	...
	// MESG("stack push [%s]",tok_info(tok0));
 	tok0_bnf=stack_push("factor",tok0,tok0->ttype,&new_one);
 };
#endif
 ...
 switch(tok0->ttype) {
	/*  the following ends factor  */
 	case TOK_SEP:
		xpos=476;
		// syntax_error(xpos,"separator in factor!");
		RT_MESG1(xpos);
	case TOK_COMMA:
	case TOK_SHOW:
	...
	case TOK_LBRAKET:{	/* array definition  */
		...
		while(1) {
			...
			if(tok->ttype==TOK_SEP) {	/* Add tok->ttype==TOK_COMMA for using comma to separate array items  */
				...
				// MESG("		new row2");
				stack_push("lbra line",tok,-tok->ttype,&new_one);
				NTOKEN_ERR(104);
				continue;
			};
		...
		stack_push("4789",tok,-tok->ttype,&new_one);
#endif
		NTOKEN_ERR(4789)
	...
	case TOK_RPAR:
		xpos=479;
		pnum--;
		// syntax_error(xpos,"wrong argument number");
		RT_MESG1(480);
	/* start of logic ---------  */
	case TOK_VAR:{	// 0 variable
		...
		if(tok->ttype==TOK_INCREASE) {
			...
			set_ttype("increase",tok0_bnf,tok->ttype);
...
#if	TBNF
			// tok0_bnf->ttype=tok->ttype;
			set_ttype("decrease",tok0_bnf,tok->ttype);
			tok0_bnf->bnf_factor_function=bnf_update_val;
			tok0_bnf->dval=-1;
#endif
			NTOKEN_ERR(4981);
		...
		if(tok->ttype==TOK_TYPE_ELEMENT) {
			...
			tok->dval=-1;
			tok_struct *bnf_tok=stack_push("type_element",tok,tok->ttype,&new_one);
			bnf_tok->dval=-1;
			NTOKEN_ERR(4982);
		};
	...
	case TOK_ARRAY_L2:{
...
#if	TBNF
		stack_push("tok_array_l2 2",tok,tok->ttype,&new_one);
#endif
		if(tok->ttype==TOK_TYPE_ELEMENT) {
			tok->dval=-1;
#if	TBNF
		...
		if(tok->ttype==TOK_DECREASE) {
			...
			tok->tgroup=0;
#if	TBNF
			stack_push("DEC_AL2",tok,TOK_DECREASE_ARRAY2,&new_one);
#endif
			NTOKEN_ERR(4985);
		};
		};
...
#endif
#if	TBNF
			stack_push("INC_AL1",tok,TOK_INCREASE_ARRAY1,&new_one);
#endif
			NTOKEN_ERR(4986);
		} else
		if(tok->ttype==TOK_DECREASE) {
			...
			set_tok_function(tok,0);
#endif
			stack_push("DEC_AL1",tok,TOK_DECREASE_ARRAY1,&new_one);
			NTOKEN_ERR(4987);
		};
		}
		RT_MESG1(494);
	...
	case TOK_ARRAY1:{
		...
		if(!check_skip_token_err1(TOK_RBRAKET,"array error",xpos)){
			...
			stack_push("RB",braket,-braket->ttype,&new_one);
#if	TBNF
			// tok0_bnf->ttype=TOK_ARRAY_L1;
			set_ttype("TOK_ARRAY1",tok0_bnf,TOK_ARRAY_L1);
...
#endif
#if	TBNF
			tok_struct *ts=stack_push("4988",tok,tok->ttype,&new_one);
	...
	case TOK_ARRAY2:{
		...
		stack_push("500",tok,-tok->ttype,&new_one);// RB
...
#if	TBNF
		stack_push("5001",tok,-tok->ttype,&new_one); //RB
		assign_var_name=tok0_bnf->tname;
#endif
...
#if	TBNF
		if(tok->ttype==TOK_ASSIGN||(tok->ttype>=TOK_INCREASEBY&&tok->ttype<=TOK_DECREASEBY)||tok->ttype==TOK_DIVBY) {
			tok0_bnf->bnf_factor_function=bnf_factor_array_l2_tba;
	...
	case TOK_LPAR:
		{
			...
			CHECK_TOK(481);
			{
				if(tok->ttype==TOK_RPAR) {	/* null expression  */
					pnum--;
					stack_push("483",tok,-tok->ttype,&new_one);
					NTOKEN_ERR(483);
	...
	case TOK_MINUS:
		...
		if(pre_symbol>1) {
			xpos=490;
			syntax_error(xpos,"too many symbols(+) infront of factor");
			RT_MESG1(490);
		};
		CHECK_TOK(491);
		if(pre_symbol) {
...
#if	TBNF
			// MESG("push negate!!!!!");
			tok0_bnf=stack_push("negate",tok0,TOK_NEGATE,&new_one);
#endif
		} else {
#if	TBNF
			// MESG("push minus!!!!!");
	...
	case TOK_PLUS:
		...
		// stack_push("NOT",tok0,tok0->ttype,&new_one);
		RT_MESG1(4911);
	case TOK_NOT:
		xpos=492;
...
#if	TBNF
		// stack_push("TOK_OPTION",tok0,tok0->ttype,&new_one);
#endif
		ex_edenv=0;
		pre_symbol=0;
		RT_MESG1(xpos);
...
#else
		stack_push("proc ) ",tok,-TOK_RPAR,&new_one);
#endif
		// MESG("	tok_proc:1");
		NTOKEN_ERR(107);
		after_proc=tok;	// this must be RPAR token!
	...
	case TOK_CMD:	{ // 3 editor command
...
#if	TBNF
#if	!TFUNC2
		stack_push("TOK_CMD",tok0,-tok0->ttype,&new_one);
#endif
#endif
		pre_symbol=0;
		...
		macro_exec=MACRO_MODE2;
		{
			...
			for(i=0;i<args;i++){
				...
				if(i<args-1) {	/* check for a comma  */
					...
					if(tok->ttype==TOK_COMMA || check_end==0) {
						...
						syntax_error(xpos,"argument error,comma expected");
			...
			if(i!=(args-1) && check_end) {
				xpos=521; 
				syntax_error(xpos,"wrong number of arguments");
				macro_exec=save_macro_exec;
				RT_MESG1(5211);
			};
			CHECK_TOK(5211);
...
#else
			if(tok->ttype==TOK_RPAR) {
				stack_push("cmd rpar",tok,-tok->ttype,&new_one);
			};
#endif
#endif
	...
	case TOK_ASSIGN_TYPE:{
		...
		// MESG("		tok [%s]",tok_info(tok));
		if(tok->ttype==TOK_LPAR) stack_push("assign type",tok,-tok->ttype,&new_one);
...
int err_num_term3(tok_struct *tok1)
{
 ...
 err_num = err_factor();
 stack_push("term3",tok1,tok1->ttype,&new_one);
 ...
 if(tok->tgroup==TOK_TERM2) {
	...
	if(tok1->ttype==TOK_MOD||tok1->ttype==TOK_POWER) {
		...
		NTOKEN_ERR(544);
		stack_push("term3 function",tok1,tok1->ttype,&new_one);
		err_num = err_num_term3(tok0);
	};
...
int err_num_term2()
{
 ...
 if(tok->tgroup==TOK_TERM2) {
	...
	if(tok0->ttype==TOK_MOD||tok0->ttype==TOK_POWER) {
		...
		stack_push("num_term2",tok0,tok0->ttype,&new_one);
...
int err_num_term1()
{
 ...
 while (tok->tgroup==TOK_TERM1) {
	...
	stack_push("num_term1",tok0,tok0->ttype,&new_one);
...
int err_num_expression()
{
 ...
 xpos=561;
 SHOW_STAGE(561);
 // MESG("- start num expression!");
 // tok_struct *op2=tok;
 err_num = err_num_term1();
 if(err_num) return(err_num);
 
 // MESG("	num expression [%s]",tok_info(tok));
 while (tok->tgroup==TOK_TERM) {
	...
	if(tok->ttype==TOK_MINUS) {	/* TOK_MINUS  */
		...
		NTOKEN_ERR(571);
		err_num=err_num_term1();
		stack_push("TOK_MINUS",tok0,tok0->ttype,&new_one);
	};
 };
 ...
 int new_one=0; 
	...
	switch(tok->ttype){
		case TOK_OR: {	/* TOK_OR  */
			...
			stack_push("TOK_OR",tok0,tok0->ttype,&new_one);
			if(err_num) return err_num;
			CHECK_TOK(706);
			simple=0;
			continue;
		...
		case TOK_XOR: {	
			...
			err_num=err_lexpression();
			// MESG_TOK_INFO("#2 err_lexpression",tok0);
			stack_push(" TOK_XOR",tok0,tok0->ttype,&new_one);
			if(err_num) return err_num;
			CHECK_TOK(712);
			simple=0;
			continue;
		...
		case TOK_NOR: {	
			...
			err_num=err_lexpression();
			// MESG_TOK_INFO("#2 err_lexpression",tok0);
			stack_push("TOK_NOR",tok0,tok0->ttype,&new_one);
			if(err_num) return err_num;
			CHECK_TOK(712);
			simple=0;
			continue;
		...
		case TOK_BIGGEREQ:
			...
			NTOKEN_ERR(708);
			err_num=err_cexpression();
			stack_push("compare",tok0,tok0->ttype,&new_one);
			simple=0;
			continue;
		case TOK_AND: {	
			...
			err_num=err_cexpression();
			// MESG_TOK_INFO("#2 err_lexpression",tok0);
			stack_push("TOK_AND",tok0,tok0->ttype,&new_one);
			if(err_num) return err_num;
			CHECK_TOK(712);
			simple=0;
			continue;
		...
		case TOK_NAND: {	
			...
			err_num=err_cexpression();
			// MESG_TOK_INFO("#2 err_lexpression",tok0);
			stack_push("TOK_NAND",tok0,tok0->ttype,&new_one);
			if(err_num) return err_num;
			CHECK_TOK(712);
			simple=0;
			continue;
		...
		case TOK_ASSIGN: {
...
#if	TBNF
			tok_struct *tok0_bnf_assign=stack_push("TOK_ASSIGN 1",tok0,tok0->ttype,&new_one);
			...
			if(assign_type_to) {
				if(assign_type_to==TOK_ASSIGN_ARRAY2) {
					...
					set_ttype("ASSIGN_ARRAY2",tok0_bnf_assign,TOK_ASSIGN_ARRAY2);
					...
					set_ttype("ASSIGN_ARRAY1",tok0_bnf_assign,TOK_ASSIGN_ARRAY1);
				...
				if(assign_type_to==TOK_ASSIGN_TYPE) {
					tok0_bnf_assign->tname="EL1=";
					// tok0_bnf_assign->ttype=TOK_ASSIGN_ARRAY1;
					set_ttype("ASSIGN_TYPE",tok0_bnf_assign,TOK_ASSIGN_TYPE);
					tok0_bnf_assign->bnf_factor_function=bnf_assign_type_el1;
					// MESG(";3 for [%s] set assign to type %d,bnf_assign_type_el1",tok_info(tok0_bnf_assign),TOK_ASSIGN_ARRAY1);
				};
				if(assign_type_to==TOK_TYPE_ELEMENT) {
					tok0_bnf_assign->tname="EL=";
					// tok0_bnf_assign->ttype=TOK_ASSIGN_ARRAY2;
					set_ttype("TYPE_ELEMENT",tok0_bnf_assign,TOK_ASSIGN_ARRAY2);
...
#if	TBNF
			tok_struct *dest=stack_push(tok0->tname,tok0,tok0->ttype,&new_one);
			if(assign_type_to) {
				if(assign_type_to==TOK_ASSIGN_ARRAY2) {
					dest->tname="A2+=";
		...
		case TOK_MULBY: {
			...
			err_num=err_mulby();
#if	TBNF
			tok_struct *dest=stack_push("*=",tok0,tok0->ttype,&new_one);
			...
			if(assign_type_to) {
				if(assign_type_to==TOK_ASSIGN_ARRAY2) {
				 ...
				 if(assign_type_to==TOK_TYPE_ELEMENT) {
					dest->tname="EL*=";
					dest->bnf_factor_function=bnf_mulby_element;
					// MESG("; for [%s] set assign to type %d,bnf_inmulby_array1",tok_info(dest));
				};
				assign_type_to=0;
			};
#endif
		...
		case TOK_DIVBY: {
			...
			if(assign_type_to) {
				if(assign_type_to==TOK_ASSIGN_ARRAY2) {
					// dest->tname="A2/=";
					dest->bnf_factor_function=bnf_divby_array2;
					// MESG("; for [%s] set assign to type %d,bnf_mulby_array2",tok_info(dest));
				}; if(assign_type_to==TOK_ASSIGN_ARRAY1) {
					// dest->tname="A1/=";
					dest->bnf_factor_function=bnf_divby_array1;
					// MESG("; for [%s] set assign to type %d,bnf_mulby_array1",tok_info(dest));
				};

				 if(assign_type_to==TOK_ASSIGN_TYPE) {
					dest->tname="EL1/=";
					dest->bnf_factor_function=bnf_divby_element;
					// MESG("; for [%s] mulby_element",tok_info(dest));
				};
				 if(assign_type_to==TOK_TYPE_ELEMENT) {
					dest->tname="EL/=";
					dest->bnf_factor_function=bnf_divby_element;
					// MESG("; for [%s] set assign to type %d,bnf_indivby_array1",tok_info(dest));
				};
				assign_type_to=0;
			};
#endif
		...
		case TOK_DECREASEBY: {
			...
			if(assign_type_to) {
				if(assign_type_to==TOK_ASSIGN_ARRAY2) {
				 ...
				 if(assign_type_to==TOK_ASSIGN_TYPE) {
					dest->tname="EL1-=";
					dest->bnf_factor_function=bnf_decreaseby_element;
					// MESG("; for [%s] decreaseby_element",tok_info(dest));
				};
		...
		case TOK_ASSIGNENV:
			...
			// stack_push_replace("tok l",tok_l,&new_one);
			stack_push("assign env",tok0,tok0->ttype,&new_one);
			RT_MESG1(7141);
		case TOK_ASSIGNOPT:
			...
			NTOKEN_ERR(7105);
			err_num=err_assign_env();
			stack_push("assign option",tok0,tok0->ttype,&new_one);
			RT_MESG1(7141);
		default:
			RT_MESG1(715);
	};
...
int err_check_sentence1()
{
 ...
 switch(tok->ttype) {
	...
	case TOK_RCURL:
		// MESG_TOK_INFO("# err_check_sentence",tok);
#if	TBNF
		stack_push("sentence RCURL",tok,-tok->ttype,&new_one);
#endif
		CHECK_TOK(626);
		break;
 	case TOK_LCURL:
...
#endif
#if	TBNF
		stack_push("sentence LCURL",tok,-tok->ttype,&new_one);
#endif
		NTOKEN_ERR(627);
		CHECK_TOK(628);
...
#if	TBNF
#if	!TFUNC
		tok_struct *if_token=stack_push("DIR_IF",tok,-tok->ttype,&new_one);
#else
		tok_struct *if1=tok;
		if1->tname="then";
#endif
...
//		if(tok->ttype==TOK_LCURL) is_block=1; 
	...
	case TOK_DIR_FORI:
...
#endif
#if	TBNF
		stack_push("DIR_FORI",tok,-tok->ttype,&new_one);
...
#if	TBNF
		tok0_bnf=stack_push("for i assign",tok0,tok0->ttype,&new_one);
		...
		CHECK_TOK(6406);
#if	TBNF
		stack_push("fori sep 1",tok,0,&new_one);
#endif
		NTOKEN_ERR(6407);	/* skip separator! */
		err_num=err_num_expression();
		if(err_num) return(err_num);
#if	TBNF
		stack_push("fori sep 2",tok,-tok->ttype,&new_one);
#endif
		NTOKEN_ERR(6408);
		err_num=err_num_expression();
		if(err_num) return(err_num);
		CHECK_TOK(6409);
#if	TBNF
		stack_push("fori )",tok,-tok->ttype,&new_one);
#endif
	...
	case TOK_DIR_FOR:
		...
		CHECK_TOK(647);
#if	TBNF
		stack_push("for )",tok,-tok->ttype,&new_one);
#endif
...
#endif
#if	TBNF
			stack_push("while",tok,-tok->ttype,&new_one);
#endif
...
#endif
#if	TBNF
		stack_push("dir_break",tok,-tok->ttype,&new_one);
#endif
		NTOKEN_ERR(663);
		RT_MESG;
	case TOK_DIR_RETURN:{
...
#if	TBNF
#if	!TFUNC
		tok_struct *dest=stack_push("dir_return",tok,-tok->ttype,&new_one);
#else
...
#if	TBNF
#if	TFUNC
			stack_push("dir_return",t_return,-t_return->ttype,&new_one);
#endif
#if	!TFUNC
...
#if	TBNF
#if	!TFUNC
		stack_push("dir_return )",tok,-tok->ttype,&new_one);
#endif
#endif
		check_skip_token1(TOK_RPAR);
	...
	case TOK_SEP:
#if	TBNF
		stack_push("sep ",tok,-tok->ttype,&new_one);
#endif
		// MESG_TOK_INFO("# err_check_sentence",tok);
		RT_MESG;
	case TOK_DIR_TYPE:
...
#endif
#if	TBNF
		stack_push("dir type",tok,-tok->ttype,&new_one);
#endif
...
int err_check_block1()
{
   ...
   while(1) {
	...
	switch(tok->ttype) {
		case TOK_EOF: 
			// MESG("err_check_block1: [%s] --> [%s]", check_buffer->b_fname,tok_info(tok));
#if	TBNF
			stack_push("EOF ",tok,-tok->ttype,&new_one);
#endif
			RT_MESG1(673);
		case TOK_SEP:
			// MESG_TOK_INFO("# err_check_block1",tok);
#if	TBNF
			stack_push(";separator",tok,-tok->ttype,&new_one);
#endif
			NTOKEN_ERR(6741);
			continue;
...
#if	TBNF
			// MESG("TOK_RPAR:");
			stack_push("RPAR ",tok,-tok->ttype,&new_one);
#endif
			NTOKEN_ERR(6741);
			continue;
		case TOK_COMMA:
			...
			tok_struct *comma = stack_push("comma",tok,tok->ttype,&new_one);
			// comma->tname=",1";
		...
		case TOK_SHOW:
			// MESG_TOK_INFO("# err_check_block1",tok);
#if	TBNF
			stack_push("Show/comma",tok,-tok->ttype,&new_one);
#endif
#if	TNORMAL
		...
		case TOK_RCURL:
#if	TBNF
			stack_push("RCURL",tok,-tok->ttype,&new_one);
#endif
			// MESG("err_check_block1: [%s] --> [%s]", check_buffer->b_fname,tok_info(tok));
			NTOKEN_ERR(675);
			RT_MESG1(6751);
 ...
 xpos=692;syntax_error(xpos,"Null in block");
 RT_MESG1(6921);
}

...
{
 gtk_statusbar_pop((GtkStatusbar *)wplot_pline,1);
...
int plot_title(num n)
{
 ...
 v = add_plot_element(PTITLE,1+strlen(s)/sizeof(double) );
...
//    cairo_show_text (crp, "Στατικός τίτλος!");
...
int plot_image(num n) 
{
 ...
 if(!plot_is_maped) return(0);
	...
	v = add_plot_element(PIMAGE,4+strlen(s)/sizeof(double));
	size = strlen(s)+1;
	v[0]=zoom;	v[1]=angle;
